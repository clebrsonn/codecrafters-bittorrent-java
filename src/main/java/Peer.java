...
import utils.PeerByteArrayOutputStream;
...
public class Peer implements AutoCloseable {
    ...
    public void send(Message message, MessageSerialContext context) throws IOException {
        ...
        final var byteArrayOutputStream = new PeerByteArrayOutputStream();
        final var length = descriptor.serialize(message, new DataOutputStream(byteArrayOutputStream), context);

        System.err.printf("send: typeId=%-2d length=%-6d message=%s%n", descriptor.typeId(), length, message);
    ...
    public byte[] downloadPiece(TorrentInfo torrentInfo, int pieceIndex) throws IOException, InterruptedException {
        ...
        final var downloadedPieceHash = DigestUtil.toSha1(bytes);
    ...
    public static Peer connect(Socket socket, Announceable announceable) throws IOException {
        ...
        try {
...

            {
                ...
                /* peer id */
                outputStream.write("cbc12233445566778899".getBytes(StandardCharsets.US_ASCII));
            }

            {
                ...
                if (!Arrays.equals(receivedProtocol, PROTOCOL_BYTES)) {
                    System.out.println(DigestUtil.bytesToHex(receivedProtocol));
                ...
                System.err.println("peer: padding: %s".formatted(DigestUtil.bytesToHex(receivedPadding)));
